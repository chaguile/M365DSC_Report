...
switch ($modo) {
    ...
    '3' {
        # El archivo de referencia esperable es el ConfigurationFile.ps1 del paso 2
        $refDefault = ''
        foreach ($cand in @((Join-Path $Root 'Export\ConfigurationFile.ps1'),
                            (Join-Path $Root 'Scripts\ConfigurationFile.ps1'),
                            (Join-Path $Root 'Export\M365DSC-Export-Main.ps1'))) {
            if (Test-Path $cand) { $refDefault = $cand; break }
        }
        if (-not $refDefault) { $refDefault = Join-Path $Root 'Scripts\ConfigurationFile.ps1' }
        do {
            $path = (Read-WithDefault (tr " Ruta del script .ps1 de referencia" " Path to the reference .ps1 script") $refDefault).Trim('"').Trim()
            if (-not (Test-Path $path)) { Write-Err (tr "No existe: $path" "Does not exist: $path") }
        } until (Test-Path $path)
...
$ExportPath = Read-WithDefault (tr " Carpeta destino del export M365DSC" " M365DSC export destination folder") (Join-Path $Root 'Export')
$OutDir     = Read-WithDefault (tr " Carpeta para el script y el certificado" " Folder for the script and the certificate") (Join-Path $Root 'Export')
...
Write-Host (tr "  2. Ejecuta el paso 4 (Export) apuntando a:" "  2. Run step 4 (Export) pointing to:") -ForegroundColor Yellow
Write-Host "     $mainFile" -ForegroundColor Yellow
if ($pfxPath) {
...
if (-not $ConfigFile -and -not $AppId) {
    ...
    if ($src -eq '1') {
        # Lo normal es el M365DSC-Export-Main.ps1 generado por el paso 3 (en Export)
        $defaultCfg = ''
        foreach ($cand in @((Join-Path $Root 'Export\M365DSC-Export-Main.ps1'),
                            (Join-Path $Root 'Scripts\M365DSC-Export-Main.ps1'),
                            (Join-Path $Root 'Export\ConfigurationFile.ps1'))) {
            if (Test-Path $cand) { $defaultCfg = $cand; break }
        }
        if (-not $defaultCfg) { $defaultCfg = Join-Path $Root 'Export\M365DSC-Export-Main.ps1' }
        do {
...
function Get-DscContentSanitized {
    ...
    return @{ Content = ($out -join "`r`n"); Removed = @($removed) }
}
...
for ($c = 0; $c -lt $ConfigPaths.Count; $c++) {
    ...
    try {
        ...
        Write-Host ""
        Write-Warn (tr "ConvertTo-DSCObject fallo en '$path':" "ConvertTo-DSCObject failed on '$path':")
        Write-Warn "  $($_.Exception.Message)"
        Write-Warn (tr "Reintentando sin recursos con scripts/ADMX embebidos que rompen el parseo..." "Retrying without resources with embedded scripts/ADMX that break parsing...")
        try {
            $san = Get-DscContentSanitized -Path $path
            if ($san.Removed.Count -eq 0) { throw (tr "no se hallaron recursos problematicos que quitar" "no problematic resources found to remove") }
            $tmp = Join-Path ([IO.Path]::GetTempPath()) ("m365dsc-" + [guid]::NewGuid().ToString('N') + ".ps1")
            $san.Content | Out-File -FilePath $tmp -Encoding UTF8
            $objects = ConvertTo-DSCObject -Path $tmp -ErrorAction Stop
            Remove-Item $tmp -Force -ErrorAction SilentlyContinue
            Write-Ok (tr "Recuperado. Omitidos $($san.Removed.Count) recurso(s): $($san.Removed -join ', ')" "Recovered. Skipped $($san.Removed.Count) resource(s): $($san.Removed -join ', ')")
        } catch {
            Write-Err (tr "No se pudo recuperar '$label'; se omite del reporte. ($($_.Exception.Message))" "Could not recover '$label'; skipping it from the report. ($($_.Exception.Message))")
            continue
        }
    }
...
# Si alguna config se omitio, realinear etiquetas/baseline a las que si parsearon
if ($allConfigs.Count -lt 2) {
    Write-Err (tr "Se necesitan al menos 2 configuraciones parseables. Abortando." "At least 2 parseable configurations are needed. Aborting.")
    return
}
if ($allConfigs.Count -ne $Labels.Count) {
    $baseWanted = if ($BaselineIndex -ge 0 -and $BaselineIndex -lt $Labels.Count) { $Labels[$BaselineIndex] } else { '' }
    $Labels      = @($allConfigs | ForEach-Object { $_.Label })
    $ConfigPaths = @($allConfigs | ForEach-Object { $_.Path })
    $BaselineIndex = [Array]::IndexOf($Labels, $baseWanted)
    if ($BaselineIndex -lt 0) { $BaselineIndex = 0 }
    Write-Warn (tr "Se compararan $($Labels.Count) configuraciones; baseline: $($Labels[$BaselineIndex])" "Comparing $($Labels.Count) configurations; baseline: $($Labels[$BaselineIndex])")
}
